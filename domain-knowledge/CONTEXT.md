...
- `RunCanvas` / `RunSidecar` — the experience-plane shared-mutable pair in `SelfcoVault` `canvas/runs/` (outside `wiki/`): the sidecar markdown IS the neutral run model (source of truth), the `.canvas` is render + human input channel. The **GateService** (`selfco-box src/gate/`) is the single write authority over gate state — surfaces (canvas **URL doors**, Claude Code **gate sessions**, future cockpit panel) are thin clients; approval is a **gated emission** (draft → preview → human-typed confirm → attributable commit), the morning-cockpit ADR-0005 grammar generalized. The run canvas's summary node carries the **attention contract** (needs-you-now / status / learnings / unknowns waterfall). ADR drafts (2026-06-11, in-vault): `experience-plane-gate-service`, `gate-approval-by-emission`, `run-banner-attention-contract`.
...
**See:** `daily-logger-architecture.md`, `selfco-vault.md`, ADR-0037 (skill telemetry), ADR-0050 (skill-metrics measurement system), ADR-0054 (standup funnel measurement), ADR-0085 (selfco vault + /vault), `bead/bead.md`, `council-review/council-review.md`.
