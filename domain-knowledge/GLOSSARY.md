...
**LLM Wiki (Karpathy)** — Andrej Karpathy's pattern for an LLM-maintained personal knowledge base: append-only `raw/` sources; an LLM-owned `wiki/` of compiled, interlinked pages; a `CLAUDE.md` schema; workflows ingest/query/lint. "Obsidian is the IDE; the LLM is the programmer; the wiki is the codebase." Versus RAG: knowledge is compiled once into a persistent artifact, not re-derived per query. Gist: `karpathy/442a6bf555914893e9891c11519de94f`. The selfco vault implements this. (ADR-0085)
