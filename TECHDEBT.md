...
|----|----------|------|----------|-------------|--------|--------|
| TD-012 | MEDIUM | process | `scripts/install-agents.sh` (surface 9) · `lego-village-pipeline`, `play-well-library`, `dealdesk` | Harness never installed in three repos founded outside `/fleet-onboard` (only `.claude/northstar.md` / `launch.json` present). Running it writes into checkouts with live worker agents, so it is operator-scheduled, not a PR. | S | open |
| TD-011 | MEDIUM | process | `decisions/northstar/README.md` (surface 1) · `dealdesk`, `foundry-recipes` · `daily-logger/src/fleet.ts` `EXCLUDED_REPOS` / `REPO_NOTES` | Two swept repos have no `.claude/northstar.md`, so surface 1 cannot register them (northstar-lint ERRORs on registered-but-absent). Seven more org repos are swept-but-unnoted since discovery went derived (core-library, diy-repair-qa-eval, resume-tailor, ojfbot, ai-mcp-stack, BigMarc, fPlatonikGolf). Each needs a ruling: northstar + notes, or `EXCLUDED_REPOS` with a reason. | S per ruling · M per northstar | open |
| TD-010 | HIGH | process | `.claude/skills/scaffold-app/SKILL.md` Step 7–8 · `.claude/skills/fleet-onboard/SKILL.md` · `scripts/hooks/suggest-skill.sh` + skill-catalog triggers · founding-act memos | Four repos were founded inside agentic sessions (foundry-recipes 04-30 bootstrap, dealdesk 08-19 `chore: repo init`, lego-village-pipeline + play-well-library 09-17 via HANDOFF-LEGO-PIPE-019 founding act 7) and none invoked `/scaffold-app` or `/fleet-onboard`; the founding memo named surface 1 only. A `suggested_after: [scaffold-app]` edge existed, but it only fires when scaffold-app itself runs — none of these sessions ran it (scaffold-frame-app had no edge at all) — and against the four real founding prompts (`chore: repo init`, `gh repo create …`, "Founding acts, in order …", "bootstrap a sibling repo …") fleet-onboard scored **0** on the main catalog. Partial fix 2026-09-24 (triggers + `suggested_after` chain + docs); the deterministic hook is still missing. | M | open |
| TD-009 | HIGH | process | harness worktree creation (`.claude/worktrees/*`) · local `main` · repo merge settings | Session worktrees are branched off *local* `main`, which by standing practice carries operator commits the ledger marks "do not push" (`implementation-notes.md`, 2026-08-03). A worktree therefore silently inherits them, and a one-file PR opened from it reads as small in the authoring session while GitHub shows the full inherited set. Nothing in the branch-creation, commit, push, or PR path surfaces the divergence. Realized 2026-08-08 twice: PR #433 (18 inherited commits, 19 files, +1528 lines) was caught only by luck — `core` allows rebase-merge only, and the inherited history's 7 merge commits made it unmergeable, forcing an inspection; PR #435 the same day was **not** caught, and 9 "do not push" commits are now on **public** `origin/main`. The merge policy (rebase-only, `allow_squash_merge:false`, `allow_merge_commit:false`) is undocumented anywhere in the repo, so sessions discover it by three failed merge calls. Fix: (a) branch topic work off `origin/main`, never local `main`; (b) pre-push/pre-PR check that every commit in `origin/main..HEAD` was authored this session; (c) move the do-not-push ledger commits off `main` to a `local/ledger` branch, which eliminates the class; (d) document the rebase-only merge policy. | M | open |
...
**Update 2026-09-24:** daily-logger's sweep (surface 2) is now DERIVED — from `gh repo list ojfbot` rather than the registry, because the registry only knows repos that already have a northstar and the org list is the superset that actually catches a fresh founding (daily-logger #280). Surface 3 collapsed into one `REPO_NOTES` map. Still hand-kept: `CLAUDE.md` ecosystem table, cockpit `REPO_META`, frame-standup lists; `scripts/fleet-manifest.mjs --check` remains shadow (ADR-0086 promotion pending).
**Effort:** M
---
...
### [high] process: session worktrees inherit local `main`, publishing commits marked "do not push"
**Location:** harness worktree creation (`.claude/worktrees/*`) · local `main` · `ojfbot/core` merge settings · `implementation-notes.md:126`
**Discovered:** 2026-08-08 (twice in one day, in two parallel sessions; the hazard itself was already written down on 2026-08-03 and not acted on)
**Description:** Session worktrees are created from *local* `main`. Local `main` is, by standing practice, ahead of `origin/main` and carries operator commits the ledger explicitly marks **"do not push"** — `implementation-notes.md:126` (2026-08-03) records a session that hit this and worked around it by *deferring two repairs*, because "a branch off local main would publish the operator's 9 unpushed commits". The hazard was correctly diagnosed, logged, and left in place.

It then materialized twice on 2026-08-08:

1. **PR #433** (`/diagram` delivery amendment) — a one-line `SKILL.md` change carried **18 inherited commits, 19 files, +1528 lines**. Caught only by accident: `core` allows rebase-merge only, and the inherited history contained 7 `Merge remote-tracking branch 'origin/main'` commits, which made the PR unmergeable and forced an inspection. Rebuilt off `origin/main` before merge.
2. **PR #435** (`/wizard` skill) — same shape, **not** caught. Nine inherited bead/deviation commits rode along from the first push and are now on **public** `origin/main` (`core` is `"visibility":"public"`). The authoring session flagged it only *after* merge, as a side effect it had not chosen.

The failure is silent at every gate a session actually passes through. `git status` is clean, the diff the session authored is one file, CI is green (the secret scan passes — these are ordinary notes, not secrets), and the PR body is written from the session's own mental model. The divergence is visible only in `git log origin/main..HEAD`, which nothing prompts anyone to run. Severity is HIGH because the realized consequence is unintended publication to a public repo of content deliberately withheld from it — and because the detector that saved #433 (rebase-only merge policy colliding with inherited merge commits) is luck, not a control: in a squash-enabled repo all 18 commits would have merged silently under a one-line commit message.

Contributing: the repo's merge policy (`allow_rebase_merge:true`, `allow_squash_merge:false`, `allow_merge_commit:false`) is **undocumented** — not in `CLAUDE.md`, `/push-all`, `/deploy`, `/git-guardrails`, or `coding-standards.md`. Sessions discover it by three failed `gh pr merge` calls, and nothing warns that merge commits in a topic branch make it unmergeable under that policy.

**Proposed fix:** Four parts, cheapest first — (a) **document the merge policy** (rebase-only + "no merge commits in topic branches") in `CLAUDE.md` and `/push-all`; (b) **branch off `origin/main`** — make `git switch -c <name> origin/main` the convention for topic work, so a worktree's base is never the local trunk; (c) **add a pre-push / pre-PR check** that every commit in `origin/main..HEAD` was authored in the current session, refusing or warning otherwise — this is the actual detector, and it belongs in `/git-guardrails` alongside the existing deny rules; (d) **move the do-not-push ledger commits off `main`** to a `local/ledger` branch, which removes the hazard class rather than guarding it. (d) is the real fix; (a)–(c) are containment.

**Effort:** M

---

### [high] process: repo founding inside agentic sessions bypasses `/scaffold-app` and `/fleet-onboard`
**Location:** `.claude/skills/scaffold-app/SKILL.md` Step 7–8 · `.claude/skills/fleet-onboard/SKILL.md` · `scripts/hooks/suggest-skill.sh` + `skill-catalog.json` triggers · handoff memos with "founding acts" sections
**Discovered:** 2026-09-24 (daily-logger silent-sweep RCA: 35 green no-op days, 08-21 → 09-24)
**Description:** Every one of the four repos the sweep had drifted past was founded *inside an agentic session*, not by hand: foundry-recipes (04-30, "phase 0: bootstrap"), dealdesk (08-19, `chore: repo init`), lego-village-pipeline + play-well-library (09-17, `HANDOFF-LEGO-PIPE-019` founding act 7 — "Register both repos in Northstar under play-well", which the session did exactly, and only that). None of those sessions invoked `/scaffold-app` or `/fleet-onboard`. Three gaps compound: (1) fleet-onboard's `suggested_after: [scaffold-app]` edge only fires when scaffold-app itself was invoked — none of these sessions invoked it, and scaffold-frame-app had no edge — so the chain never had a chance; (2) the fleet-onboard triggers were all onboarding vocabulary ("onboard this repo", "register the repo everywhere") — replaying the four real founding prompts (`chore: repo init`, `gh repo create …`, "Founding acts, in order …", "bootstrap a sibling repo …") through `scoreCatalog` against the main catalog scores fleet-onboard **0** on every one (with this PR it ranks first on all four); (3) the founding-acts memo convention has no line saying "and onboard", so a memo-driven founding stops at surface 1. This is the ADR-0068 adoption problem in a new costume: the skill existed, was correct, and was not in the path.
**Partial fix (2026-09-24, this PR):** fleet-onboard gains founding-shaped triggers and the missing `scaffold-frame-app` edge in `suggested_after`; scaffold-app / scaffold-frame-app now instruct the agent to invoke it; fleet-onboard gotcha "Founding is onboarding" records the incident.
**Remaining:** a deterministic hook — PostToolUse on `gh repo create` / a new `~/ojfbot/<dir>/.git` — that emits the fleet-onboard suggestion (or opens a bead) regardless of prompt wording; a one-line "founding act N: `/fleet-onboard <repo>`" convention in the memo/handoff templates; measure via the skill-acted telemetry whether the chain is followed.
**Effort:** M

---

### [medium] process: swept repos without a northstar or a ruling
**Location:** `decisions/northstar/README.md` (surface 1) · `dealdesk`, `foundry-recipes` · `daily-logger/src/fleet.ts` `EXCLUDED_REPOS` / `REPO_NOTES`
**Discovered:** 2026-09-24
**Description:** `dealdesk` and `foundry-recipes` are active (pushed 09-24) but have no `.claude/northstar.md`, so they cannot be registered on surface 1 without tripping northstar-lint. Since daily-logger derives its sweep from the org, seven more repos are swept-but-unnoted: core-library (last push 08-01), diy-repair-qa-eval (07-03), resume-tailor (06-17), ojfbot (04-12), ai-mcp-stack (2025-08), BigMarc and fPlatonikGolf (2023). Each run prints one `::warning::fleet drift` per repo until it gets a `REPO_NOTES` entry + prompt bullet, or an `EXCLUDED_REPOS` entry with a reason.
**Owner:** operator ruling per repo (in-fleet → author northstar + run `/fleet-onboard`; out-of-fleet → `EXCLUDED_REPOS`).
**Effort:** S per ruling · M per northstar

---

### [medium] process: surface 9 (install-agents) never run for three founded repos
**Location:** `scripts/install-agents.sh <repo>` · `lego-village-pipeline`, `play-well-library`, `dealdesk`
**Discovered:** 2026-09-24
**Description:** The harness (skills, hooks, settings) was never installed in the three repos founded outside `/fleet-onboard`; `lego-village-pipeline` and `play-well-library` have only `.claude/northstar.md`, `dealdesk` only `.claude/launch.json`. Running the installer writes into checkouts that currently have live worker agents, so it is a scheduled operator step (concurrent-agent git safety), not something a fleet-onboard backport PR should do.
**Effort:** S
