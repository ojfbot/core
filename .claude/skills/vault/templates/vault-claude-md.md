...
| `canvas/` | **The LLM** | Obsidian **JSON Canvas** (`.canvas` files) — curated, hand-arranged spatial maps (e.g. `kepano-adoption.canvas`). For the load-bearing diagrams a force-directed graph can't hold still. Also outside `wiki/`. **Size nodes to their content** (operator feedback, 2026-06-10): a file-node renders its embedded page in exactly the box you give it — ~420×310 shows title + properties, full height (~1500×2000) for a page meant to be read on-canvas; a 100px-tall strip hides everything. Text nodes: height ≥ the text's *wrapped* rendered lines (long lines wrap at the node width — count the wrap, not the logical line). **Edge labels** render as unboxed text at the gap midpoint, so a horizontal edge whose two nodes sit closer than the label is wide spills its caption under them — keep the gap ≥ the label width. Enforced by `canvas-fit.py` in the `/vault` skill — run it after authoring/editing any `.canvas` (grow-only / push-apart: grows nodes, widens left↔right label gaps, pushes overlaps down, re-expands groups; never shrinks; flags vertical/corner labels it won't auto-move). `--check` is wired into `lint.py --gate`, so an unfit canvas blocks the gate. |
