...
| `canvas/` | **The LLM** | Obsidian **JSON Canvas** (`.canvas` files) — curated, hand-arranged spatial maps (e.g. `kepano-adoption.canvas`). For the load-bearing diagrams a force-directed graph can't hold still. Also outside `wiki/`. **Open with an executive summary** (operator feedback, 2026-06-11): every canvas's topmost node is one wide text node saying what the canvas is, where it stands, and how to read it — the point must be graspable at a glance before any panning. Run canvases (`canvas/runs/`) get this node (`run-summary`) generated by the renderer; curated canvases carry it by hand. **For run canvases the summary is an attention contract** (operator feedback, same day): needs-you-now first (each waiting gate as a clickable `ojfbot://` door + how many downstream tasks it blocks), then status, then what the run has learned, then its unknowns ordered by what they block. Gate cards carry the same door link; clicking one opens a gate session (brief → deliberate → signed decision → commit). |
