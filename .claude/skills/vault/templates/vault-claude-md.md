...
| `diagrams/` | **The LLM** | Standing Mermaid diagram files written by the fleet's `/diagram` skill (operator ruling 2026-08-03: Obsidian is the norm for operator-facing standing diagrams — fleet maps, repo orientations, session-delta records worth keeping). Obsidian renders the fences natively. Follows `ojfbot/core` `domain-knowledge/diagram-conventions.md` (OPM shapes, OPL verbs, title + caption per fence, ≤15 nodes). Kept **outside `wiki/`** like `bases/` and `canvas/`, so the `wiki/`-scoped lint invariant is never touched. Distinct from `canvas/` (hand-arranged JSON Canvas spatial maps) — `diagrams/` files are text-canonical Mermaid, regenerated by sessions, never hand-arranged. |
| `teach/` | **The LLM** | The fleet's **teach corpus** — lessons and learning records deposited by `/teach` workspaces, keyed by topic slug (`teach/<topic-slug>/`), with numbered self-contained HTML lessons (`0001-<slug>.html`) and ADR-style `learning-records/*.md`. Also **outside `wiki/`**, like `bases/`, `canvas/`, and `diagrams/`, so the `wiki/`-scoped lint invariant is never touched. An **artifact sink, not a work-item surface** — nothing in it is ever open, assigned, or closed (ADR-0097 row D6; see `adr:teach-corpus-deposit-architecture`). Obsidian will **not** render a standalone `.html` as a page — a lesson is an attachment opened in a browser, so `teach/index.md` is the browsable hub, appended by the same deposit step that emits `harness:lesson-deposited` to `tracking/teach-sessions.jsonl`. A deposit that does not emit did not happen; `reconcile-teach-deposits.mjs` flags pushed `teach/*` branches with no deposit row. |
