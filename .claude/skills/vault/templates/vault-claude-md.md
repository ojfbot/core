...
This vault is mirrored to a **private GitHub repo `ojfbot/selfco`** — that mirror is what the Claude *apps* reach. **GitHub is the source of truth; this folder on the Mac is a clone** (`/vault` on the Mac pulls/pushes; `scripts/autocommit.sh` covers writes that bypass it).

- **claude.ai web · Claude iPhone app · Claude Desktop** — via Anthropic's official **GitHub connector** (scoped to `ojfbot/selfco`). Every write is a commit. `git pull` on the Mac to bring app-originated changes down.
- **Claude Desktop (Mac) also** — a local **`mcp-obsidian`** server (the Obsidian "Local REST API" plugin + `MarkusPfundstein/mcp-obsidian`) for a live Obsidian index. Its writes aren't commits — `autocommit.sh` (or `git push` by hand) handles that.
- **The `/vault` skill in the apps** — a tool-agnostic **Agent Skill** uploaded to your Anthropic account (`core/.claude/skills/vault/consumer/SKILL.md`): `ingest` / `query` / `note` / `orient` / `handoff`. It reads *this file* for the schema and writes via whatever connector is attached. `init` / `sync` / full web-`research` stay in **Claude Code on the Mac** (they need the python helpers + the ojfbot repos + git). Future: a Cloudflare-tunneled obsidian-mcp on dedicated hardware swaps in as a custom connector with no Skill change.
- **Session handoff** — `/vault handoff` (in Claude Code, or the Agent Skill) folds *the current* session into the wiki: a summary + the **full text of every artifact** produced + a `wiki/log.md` entry. To capture a session from a chat agent with *no* connector (Dia, plain claude.ai), copy the body of `prompts/session-handoff.md` into it → it emits a `--- BEGIN selfco handoff bundle ---` block → bring that back here and `/vault ingest` it (it lands in `raw/session-<date>-<slug>.md` and gets filed). Long research sessions with many drafts → the bundle captures all of them.

Setup + migration path: `core/.claude/skills/vault/knowledge/connectors.md`. Design: ADR-0069, ADR-0070.
