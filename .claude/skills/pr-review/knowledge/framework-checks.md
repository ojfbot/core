...
Project-specific invariants (shared with `/validate` — keep in sync with `../validate/knowledge/framework-checks.md`). Loaded during `/pr-review` when the PR touches one of these stacks. Supplement the universal invariants with these checks for the stack in use.
