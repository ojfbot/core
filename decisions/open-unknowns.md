...
- **That the employer's employment agreement and publication policy permit de-identified, pattern-level technical writing.** The ruling was made without either party reading the policy. If it turns out to require pre-approval for any work-derived publication, this ruling is void and the boundary collapses to employer-indifferent. This is the assumption the entire decision rests on.
...
- `core-library/public/graph/selfco.json` — 1.5 MB, untracked, **not gitignored**, in a public repo, carrying a protected term. One `git add -A` publishes it. Flagged, not fixed (different repo).
