...
| D23 | HTML lesson authoring: one self-contained numbered HTML file; Tufte-beautiful; working-memory-short with one tangible win; quiz answers length-matched ("Don't give the user any clues about the answer through formatting"); littered with citations; recommends one primary source; follow-up-questions reminder; reusable `./assets/` components, shared stylesheet first | **ABSORB — amended by #386** | `SKILL.md` "Lessons"/"Assets"/"Skills" → HTML-canonical already ruled by the map; the full authoring shape is the working spec. **#386 (2026-08-03) verified the shape and amended one detail:** "self-contained" and "shared stylesheet in `./assets/`" conflict at the render boundary. Measured — strip the sibling stylesheet and the lesson falls back to Times at an 8px body margin (0 rules loaded), losing every Tufte/printability opinion, while the inline quiz JS survives intact. `./assets/` is therefore the **authoring** source (correct as upstream has it — N lessons inherit one stylesheet); the **shipped** lesson is build output with the CSS inlined. Everything else in D23 held as written. |
...
delegation, evidence-gated learning records, records+mission ZPD floor, HTML lesson + assets shape as
amended by the #386 spike, reference/glossary split with understanding-gate); one rejected (NOTES.md — preferences
